...
QEMUBINDIR = /home/bsb/qemu/qemu/bin
...
qemu: $(OBJDIR)/bootdisk.vmi
	$(QEMUBINDIR)/qemu -fda $(OBJDIR)/bootdisk.vmi -boot a -k en-us

# --------------------------------------------------------------------------
# 'qemu-smp' ruft den qemu-Emulator mit SMP-Support und 2 CPUs mit dem System auf.

qemu-smp: $(OBJDIR)/bootdisk.vmi
	$(QEMUBINDIR)/qemu -fda $(OBJDIR)/bootdisk.vmi -boot a -k en-us -smp 2
...
qemu-gdb: $(OBJDIR)/bootdisk.vmi
	...
	$(VERBOSE) echo "target remote localhost:$$PPID" >> /tmp/gdbcommands.$(shell id -u)
	$(VERBOSE) echo "continue" >> /tmp/gdbcommands.$(shell id -u)
	$(QEMUBINDIR)/qemu -fda $(OBJDIR)/bootdisk.vmi -boot a -k en-us -s -S -p $$PPID
